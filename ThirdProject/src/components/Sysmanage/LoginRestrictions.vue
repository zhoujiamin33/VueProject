<template>
	 <el-tabs type="border-card">
	  <el-tab-pane>
	   ...
	   <el-table :data="Restrictionsyi" border @selection-change="handleSelectionChange" :header-cell-style="{background:'#eef1f6',color:'#606266'}">
	   	...
	   	<el-table-column prop="position.positionName" label="职位">
	   	</el-table-column>
	  ...
	  <el-tab-pane label="未限制用户">
		  ...
		  <el-table :data="Restrictions" border @selection-change="handleSelectionChange" :header-cell-style="{background:'#eef1f6',color:'#606266'}">
		  	<el-table-column prop="empId" label="Id">
		  	</el-table-column>
		  	<el-table-column type="selection">
		  	</el-table-column>
				...
				const _this=this
					  ...
					  updatewxz() {
					  	        this.$confirm('确定要取消限制吗?', '提示', {
					  	          ...
					  	          type: 'warning'
					  				...
					  				if (_this.chektable.length == 0) {
					  					...
					  					_this.axios.put("http://localhost:8089/threeproject/updatewxz/"+ids)
							  ...
							  xz(ids){
								  ...
								  .then(function(response) {
								  	_this.axios.get("http://localhost:8089/threeproject/selectyixz")
								  	.then(function(response) {
								  		_this.Restrictionsyi= response.data
								  		console.log(response)
								  	}).catch(function(error) {
